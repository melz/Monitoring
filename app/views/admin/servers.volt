        ...
        <div class="col-md-3">
            <ul class="list-group">
                ...
                <li class="list-group-item">
                    ...
                    <a href="/admin/servers?sid={{ server.id }}">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="panel panel-default">
                        ...
                        <pre style="font-size: 8pt">yum -y install bc parted vmstat; mkdir /var/monitor/; wget -O /var/monitor/cron.sh "http://{{ hostname }}/clients/script?key={{ details.monitor_key }}&pass={{ details.monitor_pass }}"; (crontab -l ; echo "*/5 * * * * bash /var/monitor/cron.sh") | crontab -</pre>
                        <strong>Debian/Ubuntu</strong>
                        <pre style="font-size: 8pt">apt-get install bc parted sysstat; mkdir /var/monitor/; wget -O /var/monitor/cron.sh "http://{{ hostname }}/clients/script?key={{ details.monitor_key }}&pass={{ details.monitor_pass }}"; (crontab -l ; echo "*/5 * * * * bash /var/monitor/cron.sh") | crontab -</pre>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function deleteServer(sid)
    {
        if (confirm("Are you sure you want to delete server ID "+sid+"?") == true) {
            window.location.replace("/admin/deleteServer?sid="+sid);
        }
    }
</script>
{% endblock %}